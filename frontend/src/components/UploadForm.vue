...
<template>
  <div class="space-y-8">
    ...
    >
      ...
      <div v-else>
        <svg class="w-10 h-10 mx-auto text-green-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
        <p class="text-gray-200 font-medium">{{ fileName }}</p>
        <p class="text-gray-500 text-sm mt-1">{{ fileSize }}</p>
        <p class="text-indigo-400 text-xs mt-2">Click to change file</p>
      </div>
    </div>

    <!-- Settings -->
    <div v-if="file" class="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-6">
      ...
      <div>
        ...
        <div class="flex gap-2">
          <button
            ...
            @click="grain = g.id"
            :class="[
              'flex-1 py-2 px-3 rounded-lg text-sm transition-colors border',
              grain === g.id
                ? 'bg-indigo-600 border-indigo-500 text-white'
      ...
      <div class="flex items-center justify-between">
        ...
        <button
          ...
          :class="[
            ...
            compressionEnabled ? 'bg-indigo-600' : 'bg-gray-700'
          ]"
        >
          <span
            :class="[
      ...
      <button
        @click="submit"
        :disabled="uploading"
        class="w-full py-3 px-6 bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-700 disabled:text-gray-500 rounded-lg font-medium transition-colors"
      >
